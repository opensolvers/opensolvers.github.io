...
**Video:** [Quantum ESPRESSO on BPI-F3 — stock BLAS MPI_ABORT → 1.31×](https://www.youtube.com/watch?v=guf9WCAyYPM)
...
Cross-board confirmation from [opensolvers/benchmarks](https://github.com/opensolvers/benchmarks): same `gemv_n` bug, same [easyconfigs#26444](https://github.com/easybuilders/easybuild-easyconfigs/pull/26444) fix, bit-identical residuals where measured. Newer probes below reuse RV2-built binaries where noted.

## IME (Integer Matrix Extension)

Same K1 / X60 silicon as the Orange Pi RV2 — SpaceMiT's **IME** int8 matrix unit (`smt.vmadot`) on **cluster 0 only** (cores 0–3, **512 KB shared L2**). Toolchain: [RV2 IME — `xsmtvdot`](RV2.html#toolchain-support-xsmtvdot).
...
IME is **~6–7×** RVV int8 and crests near **45 GOP/s** at 768³ — same silicon as the [RV2](RV2.html#ime-integer-matrix-extension) (**~42–48 GOP/s** DRAM full GEMM; **68 GOP/s** TCM offline-B + fused pack_a when B ≤512 KiB). Source: [benchmarks/ime](https://github.com/opensolvers/benchmarks/tree/main/ime).

**Usage:** same as RV2 — **use IME**, leave **TCM off** for real LLM weights ([how to use IME / TCM](RV2.html#how-to-use-ime-and-when-not-to-use-tcm); [best results ladder](RV2.html#best-ime-results-step-2-panel--memory-2026-09-02)).

## GPU (PowerVR BXE-2-32)

Same K1 / BXE-2-32 silicon as the [RV2](RV2.html). Vendor OpenCL/Vulkan userspace is allowlisted for **BXM-4-64** only — GPGPU on BXE is **closed**; the open Mesa `pvr` route needs a kernel rebuild and is deferred. Full write-up: [RV2 GPU](RV2.html#gpu-powervr-bxe-2-32--compute-closed) · [benchmarks/gpu](https://github.com/opensolvers/benchmarks/tree/main/gpu).
...
| [ScaLAPACK](../scientific-libs/scalapack.html) `PDSYEV` na=3000, 2×4 | **106.00 s** | stock RVV **HANG** | — |

Stock unpatched RVV: HPL, ELPA, and [Quantum ESPRESSO](../apps/qe.html) fail (`nan` / SCF abort); NumPy `eigvalsh` raises `LinAlgError`; ScaLAPACK **hangs** (same `gemv_n` story as the RV2).

## BLIS vs OpenBLAS

Same BLIS `rv64iv` build vs stock CVMFS OpenBLAS 0.3.30 (no local patched OpenBLAS on this image). Details: [BLIS](../scientific-libs/blis.html).
...
Same RV2-built r5v/scalar `libfftw3` binaries, `tests/bench` MEASURE. Details: [FFTW](../scientific-libs/fftw.html).
...
Same `md.tpr` + RV2-built `libfftw3f`, serial. Details: [GROMACS](../apps/gromacs.html).
...
Same RV2 overlay `lmp` binary, five upstream `bench/` inputs. Details: [LAMMPS](../apps/lammps.html).
...
Same mode ranking as the RV2. Hand RVV LJ microbench on this board: **1.57×** vs scalar ([Kokkos](../scientific-libs/kokkos.html)).
