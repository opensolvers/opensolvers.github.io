...
- **[ELPA on RISC-V](https://www.youtube.com/watch?v=9YVAbxyOro0)** — stock RVV returns `ev0=nan`; patched gemv_n → 34.81 s · 1.58× scalar
...
| **Custom** | Custom ISA extensions beyond standard RVV | X60 **IME** (`smt.vmadot`): **~42–48 GOP/s** full GEMM, **68** TCM micro when B fits — [RV2](boards/RV2.html#best-ime-results-step-2-panel--memory-2026-09-02); **TCM off** for LLM e2e |
...
- **[MLAS](scientific-libs/mlas.html)** — ONNX Runtime QNBit int4/int8 GEMM; m1pack ~10 GOP/s; panel loop +19% @ M=4
...
- **[ONNX Runtime](apps/onnx.html)** — real LLM decode via IME `MatMulNBits`: Qwen / SmolLM2 / TinyLlama int4 **~80–156 ms/tok @4t**; int8 Q8×16 **~140–159 ms**; panel loop +19% @ M=4; **TCM skip**
- **[llama.cpp](apps/llamacpp.html)** — Q4_0 IME vs RVV; Q8_0 **hybrid** decode; **TCM off** on RV2 (`SPACEMIT_DISABLE_TCM=1`); fork [`x60-ime-rvv`](https://github.com/opensolvers/llama.cpp/tree/x60-ime-rvv)
...
- **[ESPResSo](apps/espresso.html)** — Soft-matter P3M MD; FFTW r5v **~0%**; **local opt ~1.74×** vs EESSI on dense Coulomb
...
- **[Orange Pi RV2](boards/RV2.html)** — SpaceMiT K1, 8× X60 (RVV + IME). Fixed OpenBLAS: HPL **FAILED (`nan`) → 10.53 GFLOP/s**; IME **~42–48 GOP/s** full / **68** TCM micro; [QE](apps/qe.html) high-mem **1.46×**; [llama.cpp](apps/llamacpp.html) Q8_0 hybrid tg **6.68**; **IME on / TCM off** for LLM; GROMACS Force **3.31×**; **BXE-2-32 GPGPU closed**.
- **[Banana Pi F3](boards/F3.html)** — same K1 / X60 SoC, **3.7 GB RAM**. HPL **FAILED (`nan`) → 11.52 GFLOP/s**; IME **~45 GOP/s** @768³ (RV2 ladder up to **68** with TCM when B fits); FFTW r5v **1.60×**; GROMACS FFT **1.14×**; LAMMPS Kokkos **6.29×** (eam); NumPy DGEMM **17.51 GFLOP/s**; same GPU closure as RV2.
