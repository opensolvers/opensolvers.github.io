...
Same K1 / X60 silicon as the Orange Pi RV2 — SpaceMiT's **IME** int8 matrix unit (`smt.vmadot`) on **cluster 0 only** (cores 0–3, **512 KB shared L2**). The IME is separate from RVV: one instruction accumulates a **4×4 int32** tile from **4×8 int8** operands.

Toolchain: vendor extension **`xsmtvdot`** is assembler-only (LLVM ≥ 22, GCC ≥ 16, binutils ≥ 2.46). EESSI GCC 14.3 does not know it — we use **inline asm** either as raw `.insn` encodings or via a **patched binutils 2.46 `as`** with `smt.vmadot` mnemonics. Full write-up: [RV2 IME — toolchain support](RV2.html#toolchain-support-xsmtvdot).

Detailed `ime-bench` numbers were measured on the [RV2](RV2.html) (peak **~42 GOP/s** int8 GEMM vs ~5 GOP/s RVV int8, **~7–8×**; IME1 scale-build **+4.3%** on llama.cpp Q4_0). The BPI-F3 has the same IME hardware; pin to core 0 when running [benchmarks/ime](https://github.com/opensolvers/benchmarks/tree/main/ime). End-to-end int4 inference: [ONNX Runtime](../apps/onnx.html), [MLAS](../scientific-libs/mlas.html), and [llama.cpp](../apps/llamacpp.html).

## GPU (PowerVR BXE-2-32)

Same K1 / BXE-2-32 silicon as the [RV2](RV2.html). Vendor OpenCL/Vulkan userspace is allowlisted for **BXM-4-64** only — GPGPU on BXE is **closed**; the open Mesa `pvr` route needs a kernel rebuild and is deferred. Full write-up (measured on RV2): [RV2 GPU](RV2.html#gpu-powervr-bxe-2-32--compute-closed) · [benchmarks/gpu](https://github.com/opensolvers/benchmarks/tree/main/gpu).
